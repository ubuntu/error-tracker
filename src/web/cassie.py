...
def get_oopses_by_release (release, limit=1000):
    ...
    oopses_by_release = session.prepare('SELECT column1 FROM crashdb."ErrorsByRelease" WHERE key = ? LIMIT ? ALLOW FILTERING;')
...
def get_bucket_counts (release=None, package=None, version=None, pkg_arch=None,
                       rootfs_build_version=None, channel_name=None,
                       device_name=None, device_image_version=None,
                       period=None, batch_size=100, show_failed=False,
    ...
    if rootfs_build_version:
        ...
        releases = ['Ubuntu 14.04', 'Ubuntu 16.04',
    ...
    for key in keys:
        ...
        while True:
            try:
                result = daybucketscount_cf.get(key, column_start=start,
                                                column_count=batch_size)
...
def get_retracer_counts(start, finish):
    ...
    if finish == sys.maxsize:
        ...
        return ((date, _split_into_dictionaries(result))
                for date, result in results if date < start)
    else:
        dates = _get_range_of_dates(start, finish)
        results = retracestats_cf.multiget(dates)
        return ((date, _split_into_dictionaries(results[date]))
                for date in results)
...
def get_retracer_means(start, finish):
    ...
    timings = indexes_cf.get('mean_retracing_time', column_start=start,
                             column_finish=finish, column_count=1000,
...
def get_user_crashes(user_token, limit=50, start=None):
    ...
    try:
        if start:
            start = pycassa.util.uuid.UUID(start)
            result = useroops_cf.get(user_token, column_start=start,
                                     column_count=limit,
                                     include_timestamp=True)
        else:
            result = useroops_cf.get(user_token,
                                     column_count=limit,
                                     include_timestamp=True)
        for r in result:
...
def get_retrace_failure_for_bucket(bucketid):
    bucketretracefail_fam = pycassa.ColumnFamily(pool,
...
def get_package_crash_rate(release, src_package, old_version, new_version,
    ...
    try:
        ...
        # is column_start
        old_vers_data = counters_cf.get(old_vers_column, column_start=date,
                                        column_reversed=True, column_count=15)
    except NotFoundException:
        old_vers_data = None
    try:
        # this may be unnecessarily long since updates phase in ~3 days
        new_vers_data = counters_cf.get(new_vers_column, column_reversed=True,
                                        column_count=15)
    except NotFoundException:
    ...
    if exclude_proposed:
        try:
            # The first thing done is the reversing of the order that's why it
            # is column_start
            proposed_old_vers_data = proposed_counters_cf.get(old_vers_column,
                column_start=date, column_reversed=True, column_count=15)
        except NotFoundException:
            proposed_old_vers_data = None
        try:
            # this may be unnecessarily long since updates phase in ~3 days
            proposed_new_vers_data = proposed_counters_cf.get(new_vers_column,
                column_reversed=True, column_count=15)
        except NotFoundException:
    ...
    for day in dates[:-1]:
        ...
        if exclude_proposed and proposed_old_vers_data:
            ...
            date_proposed_crashes = 0
        try:
            previous_vers_crashes.append(old_vers_data[day] -
                                         date_proposed_crashes)
        # the day doesn't exist so there were 0 errors
        except KeyError:
