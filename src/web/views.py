...
def bucket(request, bucketid=None, hashed=None):
    ...
    if failuredata:
        ...
        c["retrace_failure_missing_ddebs"] = failuredata.get("MissingDebugSymbols", "")
...
def oops(request, oopsid):
    c = {"title": "Problem instance %s" % oopsid, "oops": cassie.get_crash(oopsid)}
    c.update(common_c())
...
def status(request):
    ...
    funcs = [
        getattr(status, x, None)
        for x in dir(status)
        if isinstance(getattr(status, x, None), FunctionType) and x.startswith("check_")
    ]
    failed_msg = ""
    for x in funcs:
        if not x():
