...
_get_published_binary_version_url = (
    _launchpad_base + "/ubuntu/+archive/primary"
    "?ws.op=getPublishedBinaries&binary_name=%s&version=%s"
    "&exact_match=true&ordered=true&status=Published"
)
_get_published_source_version_url = (
    _launchpad_base + "/ubuntu/+archive/primary"
...
_get_published_binaries_for_release_url = (
    _launchpad_base + "/ubuntu/+archive/primary/?ws.op=getPublishedBinaries&binary_name=%s"
    "&exact_match=true&distro_arch_series=%s"
)
_get_packageset_url = _launchpad_base + "/package-sets/ubuntu/%s/%s"
...
def get_versions_for_binary(binary_package, ubuntu_version):
    ...
    if is_source_package(binary_package):
        package_name = urllib.parse.quote_plus(binary_package)
        ...
        ps_url = ma_link + (
            "/?ws.op=getPublishedSources&exact_match=true&status=Published&source_name=%s&distro_series=%s"
        ...
        json_data = urllib2_request_json(pb_url, config.lp_oauth_token, config.lp_oauth_secret)
    ...
    results = set()
    dist_arch = urllib.parse.quote(_distro_arch_series % codename)
    url = _get_published_binaries_for_release_url % (binary_package, dist_arch)
    results |= set(
        [
            x["binary_package_version"]
            for x in json_request_entries(url)
            if "binary_package_version" in x
        ]
    )
...
def get_release_for_binary(binary_package, version):
    ...
    url = _get_published_binary_version_url % (
        urllib.parse.quote_plus(binary_package),
        urllib.parse.quote_plus(version),
    )
    results |= set(
        [
            get_version_for_codename(x["display_name"].split(" ")[3])
            for x in json_request_entries(url)
        ]
    )
    return results
...
def _get_most_recent_binary_version(package, release):
    ...
    if release:
        ...
        url += "&distro_arch_series=" + urllib.parse.quote(distro_arch_series)
    try:
...
def bug_is_fixed(bug, release=None):
    url = _get_bug_tasks_url % urllib.parse.quote(bug)
    if release:
...
def bug_get_master_id(bug):
    ...
    """
    url = _get_bug_dupof_url % urllib.parse.quote(bug)
    try:
...
def get_binaries_in_source_package(package_name, release=None):
    ...
    package_name = urllib.parse.quote_plus(package_name)
    ...
    try:
        ...
        total_size = int(urllib2_request_json(tsl, config.lp_oauth_token, config.lp_oauth_secret))
        while len(pbs) < total_size:
            ...
            try:
                ...
                break
            json_data = urllib2_request_json(ncl, config.lp_oauth_token, config.lp_oauth_secret)
    except KeyError:
        entries = json.loads(json_data)["entries"]
        for entry in entries:
            pbs.append(entry["binary_package_name"])
...
def get_subscribed_packages(user):
    ...
    json_data = urllib2_request_json(url, config.lp_oauth_token, config.lp_oauth_secret)
    try:
        tsl = json.loads(json_data)["total_size_link"]
        total_size = int(urllib2_request_json(tsl, config.lp_oauth_token, config.lp_oauth_secret))
...
def get_packages_in_packageset_name(release, name):
    ...
    url = _get_packageset_url % (series, name) + "?ws.op=getSourcesIncluded"
...
def is_valid_source_version(src_package, version):
    url = _get_published_source_version_url % (
        urllib.parse.quote_plus(src_package),
        urllib.parse.quote_plus(version),
    )
...
def _generate_operation(title, description, target=_ubuntu_target, tags=[""]):
    ...
    return urllib.parse.urlencode(operation)
...
def _generate_headers(oauth_token, oauth_secret):
    a = (
        ...
        'oauth_nonce="%s", '
        'oauth_version="1.0"'
    ) % (_oauth_realm, oauth_token, oauth_secret, int(oauth.time.time()), oauth.generate_nonce())

    headers = {"Authorization": a, "Content-Type": "application/x-www-form-urlencoded"}
    return headers
...
def create_bug(signature, source="", releases=[], hashed=None, lastseen=""):
    ...
    if not hashed:
        href = "https://errors.ubuntu.com/bucket/?id=%s" % urllib.parse.quote(signature)
    else:
    ...
    headers = _generate_headers(config.lp_oauth_token, config.lp_oauth_secret)
    ...
    request = urllib.request.Request(_create_bug_url, operation, headers)
...
def _generate_subscription(user):
    operation = {"ws.op": "subscribe", "level": "Discussion", "person": _person_url + user}
    return urllib.parse.urlencode(operation)
...
def subscribe_user(bug, user):
    ...
    request = urllib.request.Request(url, operation, headers)
    try:
        urllib.request.urlopen(request)
    except urllib.error.HTTPError as e:
        msg = "Could not subscribe %s to bug %s:" % (user, bug)
        print(msg, str(e), e.read(), file=sys.stderr)
