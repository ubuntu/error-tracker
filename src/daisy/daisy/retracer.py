...
def shutdown():
    ...
    for target in ('x86_64-linux-gnu', 'aarch64-linux-gnu',
                   'i686-linux-gnu', 'armv7l-linux-gnu',
                   'ppc64le-linux-gnu'):
        if os.path.exists('/usr/lib/debug/.dwz/%s' % target) and \
                os.path.islink('/usr/lib/debug/.dwz/%s' % target):
...
class Retracer:
    ...
    def update_retrace_stats(self, release, day_key, retracing_time,
        ...
        self.retrace_stats_fam.add(day_key, '%s:%s' %
                                   (release, status))
        self.retrace_stats_fam.add(day_key, '%s:%s:%s' %
                                   (release, self.architecture, status))
    ...
    def callback(self, msg):
        ...
        try:
                    ...
                    for std in (out, err):
                        for line in std.splitlines():
                            ...
                            if 'failed with exit code -9' in line:
                                ...
                                if failure_storage:
                                    self.save_crash(failure_storage, report, oops_id,
                                                    core_file)
                                give_up = True
                            ...
                            if 'Mirror sync in progress' \
                                    in line:
                                retry = True
                                break
                            # ddeb server in a sad state
                            if 'does not have a Release file' \
                                    in line:
                                retry = True
                                break
                            # ddeb server in a sad state
                            if "Release' is not signed" \
                                    in line:
                                retry = True
                                break
                    if retry:
            ...
            if stacktrace_addr_sig and not original_sas:
                ...
                self.oops_cf.insert(oops_id,
                    {'StacktraceAddressSignature': stacktrace_addr_sig})
                self.oops_cf.insert(oops_id,
                    {'RetraceStatus': 'Success'})
            else:
