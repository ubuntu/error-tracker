...
unneeded_columns = ('Disassembly', 'ProcMaps', 'ProcStatus',
                    ...
                    'JournalErrors', 'Desktop-Session', 'Env',
                    'InstalledPlugins', 'Load-Avg-1min',
                    'Load-Processes-Running-Percent', 'DiskUsage',
                    'DRM.card0-DVI-D-1', 'DRM.card0-DVI-I-1',
                    'etcconfigc00example', 'dmi.bios.date',
                    'dmi.bios.vendor', 'dmi.bios.version',
                    'dmi.board.asset.tag', 'dmi.board.name',
                    'dmi.board.vendor', 'dmi.chassis.type',
                    'dmi.chassis.vendor', 'dmi.modalias',
                    'dmi.product.family', 'dmi.product.name',
                    'dmi.product.sku', 'dmi.product.version',
                    'dmi.sys.vendor', 'CurrentDmesg',
                    # alsa
                    'UserAsoundrc', 'UserAsoundrcAsoundconf',
                    'AlsaVersion', 'AlsaDevices', 'AplayDevices',
                    'ArecordDevices', 'PciMultimedia', 'AlsaInfo',
                    'AudioDevicesInUse', 'PulseList', 'PaInfo',
                    # hookutils
                    'ProcInterrupts', 'Lspci', 'Lspci-vt',
                    'Lsusb-v', 'Lsusb-t', 'ProcModules', 'UdevDb',
                    'acpidump', 'Prtconf', 'PccardctlStatus',
                    'PccardctlIdent', 'GsettingsChanges', 'IpRoute',
                    'IpAddr', 'PciNetwork', 'IfupdownConfig',
                    'WifiSyslog', 'IwConfig', 'RfKill', 'CRDA',
                    'WpaSupplicantLog', 'Papersize', 'CupsErrorLog',
                    'Locale', 'Lpstat', 'PpdFiles', 'PrintingPackages',
                    'KernLog', 'AuditLog')
...
if __name__ == '__main__':
    ...
    # artful, cosmic, and eoan are done
    codename = sys.argv[1]
    ...
    for i in range(delta.days + 1):
        date = open_date + timedelta(days=i)
        ...
        run = 1
        if last_row == '':
            r_oopses = session.execute('SELECT * FROM "ErrorsByRelease" '
                                       'WHERE key = \'%s\' '
                                       'AND key2 = \'%s\' LIMIT 5000' %
                                       (rname, date))
            print('%s %s run: %s' % (rname, date, run))
            for r_oops_row in r_oopses:
                check_and_remove_oops(r_oops_row.column1)
                last_row = r_oops_row.column1
            run += 1
        ...
        while run < 150:
            r_oopses2 = session.execute('SELECT * FROM "ErrorsByRelease" '
                                        'WHERE key = \'%s\' '
                                        'AND key2 = \'%s\' AND column1 > %s '
                                        'LIMIT 5000' %
                                        (release, date, last_row))
            print('%s %s run: %s' % (rname, date, run))
            r_oops_row = ''
