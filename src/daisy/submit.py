...
def update_proposed_counters(_session, release, src_package, date,
                             src_version=None):
    cql_release = release.replace("'", "''")
    if src_version:
...
def create_minimal_report_from_bson(data):
    ...
    for key in data:
        ...
        if key not in ('ProcMaps', 'Stacktrace', 'Signal', 'ExecutablePath',
                       'ProblemType', 'AssertionMessage', 'StacktraceTop',
                       'Traceback', 'KernelOops', 'Failure',
                       '_PythonExceptionQualifier', 'MachineType',
                       'dmi.bios.version', 'OopsText'):
            continue
        try:
...
def submit(_session, environ, system_token):
    ...
    if not counters_update:
        counters_update = \
            _session.prepare('UPDATE "Counters" SET value = value +1 WHERE key = ? and column1 = ?')
    global proposed_counters_update
    if not proposed_counters_update:
        proposed_counters_update = \
            _session.prepare('UPDATE "CountersForProposed" SET value = value +1 WHERE key = ? and column1 = ?')
    try:
    ...
    if date and exec_path and proc_status and system_token:
        try:
            ...
            results = _session.execute(
                SimpleStatement("SELECT column1 FROM \"%s\" WHERE key = %s LIMIT 1"
    ...
    if not third_party and not automated_testing and problem_type == 'Crash':
        ...
        if version == '':
            ...
            update_counters(_session, release=release, src_package=src_package,
                            src_version=version, date=day_key)
    ...
    if 'package-from-proposed' in tags:
        ...
        if not third_party and not automated_testing \
                and problem_type == 'Crash':
            update_proposed_counters(_session, release=release, src_package=src_package,
                                     date=day_key)
            if version != '':
                update_proposed_counters(_session, release=release,
    ...
    try:
        ...
        msg = "%s: WriteTimeout with %s keys." % \
              (system_token, len(list(data.keys())))
        logger.info(msg)
...
def bucket(_session, oops_config, oops_id, data, day_key):
    ...
    if not indexes_select:
        indexes_select = \
            _session.prepare('SELECT value FROM "Indexes" WHERE key = ? and column1 = ? LIMIT 1')
    stacktrace_select = None
    if not stacktrace_select:
        stacktrace_select = \
            _session.prepare('SELECT value FROM "Stacktrace" WHERE key = ? and column1 = ? LIMIT 1')
    ...
    if 'StacktraceTop' in data and 'Signal' in data:
        ...
        if cql_addr_sig:
            try:
                stacktraces = _session.execute(stacktrace_select,
                                               [cql_addr_sig, 'Stacktrace'])
                stacktrace = [stacktrace[0] for stacktrace in stacktraces][0]
                threadstacktraces = _session.execute(stacktrace_select,
                                                     [cql_addr_sig, 'ThreadStacktrace'])
        ...
        if crash_sig and not retry:
            ...
            if addr_sig:
                _session.execute(
                    SimpleStatement("INSERT INTO \"%s\" (key, column1, value) VALUES ('%s', '%s', '%s')"
