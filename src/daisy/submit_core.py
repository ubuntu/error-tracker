...
def write_to_swift(environ, fileobj, oops_id, provider_data):
    ...
    swift_cmd = [
        "/usr/bin/swift",
        "--os-auth-url",
        "%s" % provider_data["os_auth_url"],
        "--os-username",
        "%s" % provider_data["os_username"],
        "--os-password",
        "%s" % provider_data["os_password"],
        "--os-tenant-name",
        "%s" % provider_data["os_tenant_name"],
        "--os-region-name",
        "%s" % provider_data["os_region_name"],
        "--os-project-name",
        "%s" % provider_data["os_tenant_name"],
        "--auth-version",
        "3.0",
...
def write_to_storage_provider(environ, fileobj, uuid):
    ...
    provider = None
    for key, ranges in config.write_weight_ranges.items():
...
def submit(_session, environ, fileobj, uuid, arch):
    global oops_select
    if not oops_select:
        oops_select = _session.prepare(
            'SELECT value FROM "OOPS" WHERE key = ? AND column1 = ? LIMIT 1'
        )
    try:
    ...
    if not message:
        ...
        msg = "Failure to write OOPS %s to storage provider" % (uuid)
        logger.info(msg)
    ...
    try:
        addr_sig_results = _session.execute(
            oops_select, [uuid, "StacktraceAddressSignature"]
        )
    ...
    if addr_sig and queued:
        ...
        if not indexes_insert:
            indexes_insert = _session.prepare(
                'INSERT INTO "Indexes" \
                (key, column1, value) VALUES (?, ?, ?)'
            )
        _session.execute(indexes_insert, ["retracing", addr_sig, "0x"])
