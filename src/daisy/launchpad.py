...
if (
    ...
    raise ImportError(
        "You must set lp_oauth_token and " "lp_oauth_secret in local_config"
    )
...
def _get_pocket_for_binary_version(package, version, release):
    if release == "Ubuntu RTM 14.09":
        url = _get_published_binaries_url.replace(
            "/ubuntu/", "/ubuntu-rtm/"
...
def get_binaries_in_source_package(package_name, release=None):
    ...
    try:
        ...
        total_size = int(
            urllib2_request_json(tsl, config.lp_oauth_token, config.lp_oauth_secret)
        )
        while len(pbs) < total_size:
            ...
            try:
                ...
                break
            json_data = urllib2_request_json(
                ncl, config.lp_oauth_token, config.lp_oauth_secret
            )
    except KeyError:
        entries = json.loads(json_data)["entries"]
        for entry in entries:
