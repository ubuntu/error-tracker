...
if __name__ == "__main__":
    ...
    system_lookup_stmt = session.prepare('SELECT * FROM "UserOOPS" WHERE key=?')
    ...
    for i in range(max_retries):
        period = 30 + (30 * i)
        try:
            system_data = session.execute(system_lookup_stmt, [target_systemid])
    ...
    for oopsid in crashes:
        ...
        if systemid != target_systemid:
            continue
            print("systemid for %s doesn't equal the target systemid" % oopsid)
        max_retries = 5
        for i in range(max_retries):
            period = 30 + (30 * i)
            if not dry_run:
                try:
                    ...
                    print("%s%s was from %s and had its data removed" % (URL, oopsid, systemid))
                    break
                except (OperationTimedOut, NoHostAvailable):
