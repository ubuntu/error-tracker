...
class TestCrashSubmission(TestSubmission):
    ...
    def test_python_submission(self):
        ...
        self.assertEqual(
            pycassa.util.uuid.UUID(oops_id),
            list(bucket_cf.get(crash_signature).keys())[0],
        )
        ...
        for key in keys:
            ...
            for count in counts:
                self.assertEqual(list(count[1].values()), [1])
...
class TestBinarySubmission(TestCrashSubmission):
    ...
    def test_binary_submission_not_retraced(self):
        ...
        oops_id = oops_cf.get_range().next()[0]
        self.assertEqual(
            list(awaiting_retrace_cf.get(self.stack_addr_sig).keys())[0], oops_id
        )
    ...
    def test_binary_submission_already_retraced(self):
        ...
        oops_id = oops_cf.get_range().next()[0]
        bucket_contents = list(bucket_cf.get("fake-crash-signature").keys())
...
class TestCoreSubmission(TestSubmission):
    ...
    def test_core_submission(self):
        ...
        environ = {
            "QUERY_STRING": "uuid=%s&arch=amd64" % uuid,
            "CONTENT_TYPE": "application/octet-stream",
            "wsgi.input": core_io,
