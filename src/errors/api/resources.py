...
class MostCommonProblemsResource(ErrorsResource):
    ...
    def obj_get_list(self, bundle):
        ...
        for package in packages:
            buckets = cassie.get_bucket_counts(
                release,
                package,
                version,
                pkg_arch,
    ...
    def obj_create(self, bundle):
        ...
        if "LaunchpadBug" in metadata:
            ...
            releases = set(
                [
                    r[0].replace("Ubuntu ", "")
                    for r in cassie.get_versions_for_bucket(bucket)
...
class RateOfCrashesResource(ErrorsResource):
    increase = fields.BooleanField(attribute="increase", null=True, readonly=True)
    difference = fields.IntegerField(attribute="difference", null=True, readonly=True)
    web_link = fields.CharField(attribute="web_link", null=True, readonly=True)
    previous_period_in_days = fields.IntegerField(
        attribute="previous_period_in_days", null=True, readonly=True
    )
    previous_average = fields.IntegerField(attribute="previous_average", null=True, readonly=True)
...
class PackageVersionNewBuckets(ErrorsResource):
    ...
    def _handle_request(self, bundle):
        ...
        if not launchpad.is_valid_source_version(src_package, previous_version):
            raise NotFound(
                "%s version %s was not found in Launchpad." % (src_package, previous_version)
            )
        if not launchpad.is_valid_source_version(src_package, new_version):
            raise NotFound(
                "%s version %s was not found in Launchpad." % (src_package, new_version)
            )
        results = []
...
class OopsResource(ErrorsResource):
