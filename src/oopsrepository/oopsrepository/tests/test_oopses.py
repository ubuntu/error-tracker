...
class ClearCache(TestCase):
    def setUp(self):
        ...
        self.pool = pycassa.ConnectionPool(self.config['keyspace'],
                                           self.config['host'],
...
class TestPrune(ClearCache):
    ...
    def test_fresh_oops_kept(self):
        day_key = oopses.insert(self.config, 'key',
            json.dumps({"date":time.time(), "URL":'a bit boring'}))
    ...
    def test_old_oops_deleted(self):
        ...
        oopses_cf = pycassa.ColumnFamily(self.pool, 'OOPS')
        oopses_cf.insert('key',
            {"date":json.dumps(datestamp), "URL":'a bit boring'})
        oopses.prune(self.config)
...
class TestInsert(ClearCache):
    ...
    def _test_insert_check(self, oopsid, day_key, value=None):
        ...
        self.assertEqual([oopsid], oops_refs.values())
    ...
    def test_insert_unicode(self):
        ...
        oops = {'duration': u'♥'}
        day_key = oopses.insert_dict(self.config, oopsid, oops)
        self._test_insert_check(oopsid, day_key, value=u'♥')
    ...
    def test_insert_updates_counters(self):
        ...
        self.assertEqual([1], oops_count.values())

        oopsid = str(uuid.uuid1())
        day_key = oopses.insert_dict(config, oopsid, oops, user_token)
        oops_count = counters_cf.get('oopses', [day_key])
        self.assertEqual([2], oops_count.values())

class TestBucket(ClearCache):

    def test_insert_bucket(self):
        keyspace = self.useFixture(TemporaryOOPSDB()).keyspace
        fields = ['Ubuntu 12.04', 'Ubuntu 12.04:whoopsie',
                  'Ubuntu 12.04:whoopsie:3.04', 'whoopsie:3.04']
        oopsid = str(uuid.uuid1())
        oops = json.dumps({'duration': 13000})
        ...
        self.assertEqual([pycassa.util.uuid.UUID(oopsid)], oops_refs.keys())
        self.assertEqual(
            daybucketcount_cf.get(day_key, ['bucket-key']).values(), [1])
        ...
        for field in fields:
            for resolution in resolutions:
                ...
                self.assertEqual(
                    daybucketcount_cf.get(k, ['bucket-key']).values(), [2])
        for resolution in resolutions:
            self.assertEqual(
                daybucketcount_cf.get(resolution, ['bucket-key']).values(),
                [2])
    ...
    def test_update_bucket_metadata(self):
        ...
        # Does not exist yet.
        oopses.update_bucket_metadata(self.config, 'bucket-id', 'whoopsie',
                                      '1.2.3', apt.apt_pkg.version_compare,
                                      'Ubuntu 12.04')
        ...
        oopses.update_bucket_metadata(config, 'bucket-id', 'whoopsie', '1.2.4',
        ...
        oopses.update_bucket_metadata(config, 'bucket-id', 'whoopsie', '1.2.4~ev1',
                                      apt.apt_pkg.version_compare,
                                      'Ubuntu 12.04')
        metadata = bucketmetadata_cf.get('bucket-id')
        self.assertEqual(metadata['Source'], 'whoopsie')
        ...
        oopses.update_bucket_metadata(config, 'bucket-id', 'whoopsie', '1.2.2',
                                      apt.apt_pkg.version_compare,
                                      'Ubuntu 12.04')
        metadata = bucketmetadata_cf.get('bucket-id')
        self.assertEqual(metadata['Source'], 'whoopsie')
    ...
    def test_bucket_hashes(self):
        ...
        v = hashes_cf.get('bucket_%s' % h[0], columns=[h]).values()[0]
    ...
    def test_update_bucket_versions(self):
        ...
        d = bv_full.get(('bucket-id', 'Ubuntu 12.04', '1.2.3')).items()[0]
    ...
    def test_dpkg_comparator(self):
        ...
        self.assertEqual(1,
            bv_count.get('bucket-id', column_count=1)[('release', '1.0~ev1')])
        bv_count.add('bucket-id', ('release', '1.0+ev1'))
        c = [('release', '1.0+')]
        self.assertEqual(1,
            bv_count.get('bucket-id', column_start=c)[('release', '1.0+ev1')])
    ...
    def test_update_errors_by_release(self):
        ...
        oopses.update_errors_by_release(self.config, oops_id, system_token,
                                        release)

        d = firsterror.get(release, columns=[system_token])[system_token]
        self.assertEqual(today, d)
        d = errorsbyrelease.get((release, today)).values()[0]
        self.assertEqual(today, d)
    ...
    def test_update_source_version_buckets(self):
        ...
        oopses.update_source_version_buckets(self.config, src_package,
                                             version, oops_id)

        bucket_id = srcversbuckets.get((src_package, version)).keys()[0]
    ...
    def test_update_bucket_systems(self):
        ...
        system_token = 'system-id'
        version = '1.0'
        oopses.update_bucket_systems(self.config, bucketid, system_token,
                                     version=version)

        system = bv_systems.get((bucketid, version)).keys()[0]
